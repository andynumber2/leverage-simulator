...
status_note: "RESOLVED. Fix applied, verified in Playwright WebKit and Chromium, and CONFIRMED by the user in real Safari on 2026-08-26. 08-05 Task 3's human-verify gate (SHARE-04) is satisfied."

next_action: "None. Session archived to resolved/ and appended to the knowledge base."

human_verification:
  confirmed: true
  date: 2026-08-26
  browser: "real Safari (not Playwright WebKit)"
  build_identity: |
    Verified against the POST-FIX bundle. The user confirmed
    `http://localhost:4173/assets/index-vwbqaOH_.js` in the page's resource list, which is the
    build containing the `destination-over` composite. Build identity was checked rather than
    assumed, because the service worker can serve a stale precache indefinitely.
  service_worker_handling: |
    The service worker was eliminated by REMOVING THE SITE via Settings > Privacy >
    Manage Website Data, not by an unregister control. See the Environment section: the
    Develop > Service Workers path does NOT unregister anything.
  result: "Confirmed fixed. Tests passed."
...
- The app registers a `vite-plugin-pwa` service worker that precaches `index.html`, so a stale
  ...
  user MUST defeat that worker first, or the result is meaningless.

- **HOW TO DEFEAT THE SERVICE WORKER IN SAFARI (corrected, established the hard way).**
  This session first told the user to unregister the worker via **Develop > Service Workers**.
  **That instruction is WRONG.** That menu opens the worker's own Web Inspector window. It has no
  unregister control at all, so following it wastes time and leaves the stale worker in place,
  which then silently invalidates whatever the user reports back. The two paths that actually
  work are:
  1. **Settings > Privacy > Manage Website Data > select the origin > Remove.** This is what the
     user actually used for the confirming run. It clears the registration along with the site
     data.
  2. **Develop > Disable Service Workers.** This bypasses the worker for the session rather than
     removing it, which is enough for a one-off verification.
  Also confirm the served build's identity independently (read the hashed bundle filename out of
  the page's resource list and match it against the local `dist/`), because "I cleared it" is not
  evidence that the new build is the one running.
...
- timestamp: 2026-08-26
  checked: |
    HUMAN VERIFICATION in real Safari, which is the gate Playwright WebKit cannot close
    (08-05 Task 3 / SHARE-04 names Safari specifically).
  found: |
    User reports "confirmed fixed" and that the tests passed. Two controls make the report
    trustworthy rather than merely optimistic:
      (a) BUILD IDENTITY was verified, not assumed. The user read
          `http://localhost:4173/assets/index-vwbqaOH_.js` out of the page's resource list, which
          is the post-fix bundle carrying the `destination-over` composite.
      (b) THE SERVICE WORKER WAS ELIMINATED, by removing the site under Settings > Privacy >
          Manage Website Data before loading. This session's earlier instruction to unregister via
          Develop > Service Workers was wrong: that menu only opens the worker's Web Inspector and
          exposes no unregister control. Corrected in the Environment section.
  implication: |
    The fix is confirmed in the browser the bug was reported against, against the correct build,
    with the stale-precache confound removed. The session is resolved.
...
verification: |
  ...
  - CONFIRMED IN REAL SAFARI by the user, 2026-08-26. Verified against the post-fix bundle
    `assets/index-vwbqaOH_.js` (read out of the page's resource list, not assumed), with the
    service worker eliminated beforehand by removing the site under Settings > Privacy >
    Manage Website Data. 08-05 Task 3 / SHARE-04's human-verify gate is satisfied.
...
oracle_type: derived
oracle_note: |
  The assertion is not "does it look right" and not a screenshot baseline. It decodes the exported
  blob back in-page and counts non-background pixels inside each live canvas's bounding box,
  measured in EXPORT coordinates, plus a capture-1-vs-capture-2 byte-determinism check. Both are
  derived from the contract (an export must contain the canvas pixels, and two captures of an
  unchanged region must be identical), so they hold without a golden image and cannot be satisfied
  by a blank or a stale frame. The determinism half is what caught the non-transparent 1x1
  placeholder, which no blankness assertion would have flagged.

## Prevention (blameless postmortem)

**Why was this not caught?**

No gate existed for this class. The `app` browser project was Chromium-only
(`vitest.config.ts`, `instances: [{ browser: 'chromium' }]`), so the entire category of
WebKit-specific rasterization behavior was outside every automated gate the repo had. This is not
a test that was written badly or a review that missed something. Chromium executes the identical
pre-fix code correctly, so no amount of Chromium coverage could have surfaced it. The gap was
structural, and it was a known-and-deferred one: 08-RESEARCH.md had already flagged adding a
WebKit instance as a deferred scope decision. This bug is what converted that deferral into a
decision.

**Proof the gap was structural, not incidental:** the new regression file was run against the
ORIGINAL buggy `png-export.ts` in both engines. WebKit: 2 failed / 3 passed, with the exact
diagnostic. Chromium against that SAME buggy code: 5 passed. A Chromium-only gate could not have
caught this, no matter how the test was written.

**Recurrence guard:**
- `tests/app/export-png-canvas-fidelity.browser.test.ts` (5 tests), plus a new `app-webkit`
  vitest project scoped to that ONE file, run via `npm run test:app:webkit`.
- Red-phase proven (2/5 fail against the original code in WebKit), so the guard is known to bite
  rather than assumed to.

**Scope boundary on the WebKit adoption, recorded deliberately:**
The decision was "scoped WebKit", not "WebKit everywhere", and the boundary is evidence-based:
- Scoped `app-webkit` project, canvas-fidelity file only: **2.9s**, 5/5 green. Affordable.
- Full `app` suite under WebKit: **52 of 189 fail**, dominated by two causes that have nothing to
  do with this bug: WebKit's `history.replaceState` rate limit (which Chromium does not enforce)
  and `__screenshots__` baselines that were generated under Chromium.
Widening the WebKit project to the whole `app` suite is therefore a separate piece of work
requiring WebKit-generated screenshot baselines and a fix for the permalink write rate. Do not
widen the `app-webkit` project's `include` glob without doing that work first; it will turn a
2.9s green gate into a 52-failure red one.

## Follow-up candidates (logged, deliberately NOT fixed in this session)

- **WebKit `history.replaceState` rate limit. Follow-up candidate, not a defect closed here.**
