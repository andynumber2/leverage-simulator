...
**Milestone:** v1.0, closed 2026-08-27
**Outcome:** 71 of 72 requirements satisfied.
**Not met:** PERF-03 only, escalated by decision with five levers measured and refuted. The 1000ms
budget was never relaxed to reach that position. Full measurement history is in
`.planning/milestones/v1.0-ROADMAP.md`, `.planning/v1.0-MILESTONE-AUDIT.md`, and STATE.md's
Deferred Items section.

**Requirement outcomes worth carrying forward:**

- **PERF-03 (NOT MET, escalated):** a 10,000-cell sweep measures 1257 to 1411ms against a 1000ms
  budget on the 4-core D-17 baseline and has never passed. Refuted levers: WASM (1.20x slower),
  worker width 4 (zero, reverted), kernel per-bar output arrays (1-2%), pool and dispatch overhead
  (0.14ms across the whole grid), per-cell kernel compute (median ratio 1.0036). Only D-03's
  coarser default grid remains and it changes what is measured rather than making it faster.
- **VALID-01/02 (met, with a repriced tolerance):** the UPRO/TQQQ tracking gate passes, but
  TRACKING_ERROR_TOLERANCE was recomputed from 0.66% to 3.955% after measurement showed 352 of the
  395.5bp came from a real fund-NAV-versus-market-close pricing basis in the reference data itself,
  not from model error. No cost parameter was changed to achieve this. TQQQ's remaining margin is
  about 11%, which is real but not generous.
- **SHARE-04 (met, after two post-merge defects):** PNG export shipped with a WebKit-only blank
  canvas defect caught by a human gate, and a main-thread long task 2.4x over the PERF-07a budget
  caught only once CI ran on 4 cores. Both fixed before close.
